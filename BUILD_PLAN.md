...
**Delivers:**
- The single-call judge+generate path from brief §3.3: given a filtered diff plus the user's concept-tag history (from the Phase 1 data layer), shell out to `claude -p "<prompt>" --output-format json --allowedTools "" --max-turns 1` and parse the response into zero, one, or two questions (concept + instance, or instance alone if the concept tag was already answered — brief §3.2's memoization check).
