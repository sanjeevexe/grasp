...
3. Confirm `onSessionComplete` fires once per conversational turn (the `Stop` hook's actual granularity — see [DECISIONS.md](DECISIONS.md#2026-08-04--stop-fires-once-per-conversational-turn-not-once-per-whole-cli-session)), not once per tool call, and not once for a whole multi-turn interactive session.
...
2. ~~Confirm the cost cap and question-count cap remain independently distinguishable after the fact...~~ **Obsolete** — the dollar-cost cap (`costCapUsd`) was removed in the reliability rework (see [DECISIONS.md](DECISIONS.md#2026-08-12--remove-costcapusd-and-the-unknown-cost-halt-mechanism-keep-cost_usdcost_unknown-as-informational-metadata)); `questionsPerSessionCap` is now the sole generation cap, so `cap_reached` is unambiguous and this distinguishing step no longer applies. Generation itself also moved from once-per-`PostToolUse` to at most once per `Stop` (once per turn, batched, retried on failure) — see [DECISIONS.md](DECISIONS.md#2026-08-12--batched-at-stop-generation-resolved-tracking-retry-scope-and-slot-mutex-composition).
