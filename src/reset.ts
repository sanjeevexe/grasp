  ...
  try {
    ...
    if (!skipConfirm) {
      const confirmed = await askYesNo(
        `This will permanently delete ${counts.events} event row(s), ${counts.conceptTags} concept_tags row(s), ${counts.scanProgress} scan_progress row(s), and ${counts.scanFileHashes} scan_file_hashes row(s) (grasp scan will start over from the beginning) from your Grasp history. This cannot be undone. Continue? [y/N] `
      );
      if (!confirmed) {
    ...
    process.stdout.write(
      `Deleted ${deleted.events} event row(s), ${deleted.conceptTags} concept_tags row(s), ${deleted.scanProgress} scan_progress row(s), and ${deleted.scanFileHashes} scan_file_hashes row(s). History is now empty.\n`
    );
  } finally {
    db.close();
  }
