...
- [ ] Give Claude Code a task that makes it edit several files in one turn (several tool calls close together, no interactive back-and-forth in between). Confirm you get ONE combined question afterward (`grasp review` shows one entry covering all the changed files), not one question per file/tool call — this is the actual reliability fix this build shipped: generation now happens at most once per turn, not once per tool call.
...
- [ ] Lower the question cap to something small (`grasp set questions-cap 2`, or hand-edit `"questionsPerSessionCap"`) and confirm generation stops once you hit that many real questions in one sitting — and confirm the `Stop` message names the cap directly ("You've hit this session's question cap (2) — start a new Claude Code session, or run `grasp set questions-cap <n>` to raise it.").
