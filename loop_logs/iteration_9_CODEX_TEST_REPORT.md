...
Grasp builds and its basic sequential workflow works. It captured staged, unstaged, and untracked changes; filtered several simple kinds of noise; called a mock Claude CLI with the promised restrictions; accumulated limits across multiple turns in one session; showed a readable review screen; rejected blank answers; saved real answers and deliberate skips; remembered an answered concept across repositories; and made the basic soft/hard gate decision correctly when run from the repository root.

This run nevertheless found multiple release-blocking defects, including regressions already identified in the previous report. The most serious is that overlapping hooks can all claim the same edit and all make a paid Claude call. Eight simultaneous hooks created eight copies of one question, spent $0.016 despite a $0.003 setting, and exceeded a two-question limit without logging any cap miss. Repository overrides also disappear when Claude's working directory is a subdirectory, so hard mode and both safety limits can be bypassed.
...
- Every Grasp command used disposable homes and repositories under `.codex_test_scratch/`. Mock binaries, logs, configs, Git objects, and SQLite databases were all kept there. The developer's real `~/.grasp/history.db` was never opened or inspected.
...
- The scratch tree was removed after testing. The `npm install` lockfile change was also restored. Pre-existing `loop_logs/` content was left untouched.

## 1. Build, automated tests, packaging, and basic CLI — PASS

`npm install` and `npm run build` succeeded. All 51 automated tests passed. The built CLI printed version `0.1.0`, displayed help, created its defaults and database only in the scratch home, and ran its commands. `npm pack --dry-run --json` also succeeded and listed the expected executable and supporting files.
...
- A lockfile-only edit was recorded as `baseline_ignore` and did not call Claude.
- A whitespace-only reindent was recorded as `formatting_only` and did not call Claude.
- A configured `scripts/` ignore rule produced `user_ignore_pattern`, made no Claude call, and did not leak into another repository.
- Grasp's own `.grasp.json` and `.claude/settings.local.json` were excluded from the question prompt.
...
- **Edited nested renames can lose their size.** Git reported `2/2 src/{rename-source-verbose.ts => rename-target-verbose.ts}` and an `R069` rename. Grasp stored that rename as `+0/-0` and filtered it as below the size threshold because its statistics parser does not expand Git's compact brace-form rename path.
- **Obvious generated code is not recognized.** A six-line `src/api.generated.ts` beginning `AUTO-GENERATED FILE. DO NOT EDIT.` passed filtering and caused a paid call. The implementation has only a small fixed path list, while the README broadly says generated files are excluded.
- **Common formatter wrapping is not recognized.** Reformatting one function call from one line to several lines passed as meaningful and caused a paid call. The detector only handles formatting where the removed and added lines remain a one-for-one sequence after whitespace normalization.

## 3. Question generation and concept memoization — FAIL

### What worked

- Grasp invoked one command with `-p`, JSON output, empty allowed tools, one maximum turn, and a 20-second subprocess timeout.
- The prompt contained the significant diff and the answered-concept list, without full-repository access.
- A normal mock response produced concept and instance questions in the right order. A model decline stored the local diff summary and cost without creating a pending question.
- After a `validation-branching` question was genuinely answered in repository A, a response using that tag in repository B was forced to instance-only. Cross-repository memoization works.
- Error envelopes, missing costs, malformed responses, and process failures were covered by the automated suite or hook-level tests and were recorded conservatively.
...
The real Ink interface was exercised in pseudo-terminals with multi-event batches.
...
- Doing nothing did not dismiss a question.
...
- Hard mode denied the session that owned the pending question, did not deny an unrelated session, and stopped denying immediately after answer or skip.
...
- A child-process failure returned hook exit code 0, logged `miss_reason: error`, and created no pending question.
- A Claude error envelope logged `error` and preserved its known $0.007 cost.
- A hanging mock was terminated after about 20 seconds, returned normally, and logged `miss_reason: timeout` with no pending question.
- A session that never received `Stop` did not hang the database or prevent other sessions from operating.

### What is broken

A syntactically valid config containing `"ignorePatterns": "scripts/"` caused `config.ignorePatterns.some is not a function`. The production hook exited 1 with a stack trace. Worse, Grasp had already advanced its checkpoint before the asynchronous change handler failed. After the config was fixed, retrying captured only the config correction as an ignored diff; the code change had no capture row or event row and was permanently lost.
...
- Valid JSON values are not checked for correct types, allowed choices, or sensible ranges. Wrong types can crash a hook and lose work; misspelled modes or negative limits can be silently accepted.

## 9. Fresh-eyes code and documentation review — FAIL

The code is generally readable, and the documentation is commendably direct about local storage, Claude usage, manual review, no answer grading, and the single generation path. Several material claims are still false or incomplete:
