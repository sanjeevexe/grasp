...
If a generation attempt genuinely fails (a timeout or an error talking to `claude`, rather than a normal "not worth asking"), Grasp says so directly in the same nudge message: "A comprehension question failed to generate (timeout) — it'll retry automatically on this session's next turn, or run `grasp retry` now." A failed attempt's diffs are never lost — they stay unresolved and are automatically folded into the next batch, whether that's this same session's next turn or a manual `grasp retry`. Run `grasp retry` any time you want to force that retry immediately instead of waiting — it gathers every unresolved diff for the current repo across every session (including ones that have long since ended), so it also covers a diff whose original Claude Code session is gone for good, not just the one you're currently sitting in.
...
This walks your repo's tracked files (the same `.gitignore`-respecting, ignore-pattern-and-generated-file-aware filtering the diff side already uses), asks a concept-then-instance question about the files (or file sections — see below) it judges worth asking about, and presents each one immediately in the same interactive review UI `grasp review` uses — answer or skip, same flow, same sample-answer-afterward, same Escape-for-a-hint-then-one-retry. The instance question shows the specific lines it's actually about, using the file's real line numbers even for a large file split into sections; the concept question, as always, stands on its own with no code shown. Nothing here touches your diff-question history or vice versa — `grasp scan` never shows a pending diff question, and `grasp review` never shows a pending scan question — but **what you've already demonstrated understanding of is shared**: a concept you've mastered via a real Claude Code diff is never re-asked during a scan, and a concept you master during a scan is never re-asked the next time an agent's diff touches it.

**Large files are split into sections (chunks), not treated as one all-or-nothing unit.** A file gets one question per ~400-line section rather than a single shot at the whole thing, so a genuinely big file — the kind where a large fraction of the real logic actually lives — gets proportionally more chances at a real question instead of the same "one question, forever" treatment as a five-line utility file. A small file is just one section. Sections are covered independently and resumably: a second `grasp scan` run picks up any section it didn't get to, not just whole files it hadn't started. A file over a much larger, purely defensive ceiling (tens of thousands of lines — well beyond any legitimate hand-written source file) is refused entirely rather than chunked, as a guard against pathological input (a vendored dump, a checked-in bundle) rather than real source — you'll see a plain message (`"Skipped <path> — N lines, over the M-line processing ceiling."`) when that happens, and the file is still marked fully scanned so it isn't retried every run.

A `grasp scan` run stops once it's generated `scanQuestionsCap` real questions (default 15 — see [Configuration](#configuration)), so one run is a bounded, sit-down-and-do-it session, not an unbounded sweep of your whole codebase. It works through directories round-robin (a little from each top-level folder, cycling) rather than exhausting one folder before moving to the next, so a capped run always gives you a spread across the codebase instead of getting stuck entirely inside whichever folder happens to sort first — and for a large, multi-section file specifically, that same round-robin now advances it one section per pass rather than working through all of its sections back-to-back before moving on to anything else. Every scan is resumable: Grasp remembers which sections it's already looked at. A fully-covered file's content is also re-checked (by content hash, not just "have I seen this file before") each time it comes up as already-done — a genuine hand-edit clears that file's coverage and lets it re-enter the walk, while a trivial change (whitespace, etc.) is silently absorbed with no re-scan; see [Status and limitations](#status-and-limitations) for the practical size limit on this. Once every tracked file's every section has been scanned (and nothing's changed since), Grasp tells you plainly that there's nothing left, with a pointer to `grasp reset history` if you want to start fresh (that command clears scan progress along with your question/answer history — see [Commands](#commands)).
...
- **`grasp retry`** — manually retries every unresolved captured diff for the current repo (a diff whose generation attempt errored or timed out), across every session, not just a still-live one. If nothing's unresolved, it says so and exits cleanly. See [Using it](#using-it) above for the full failure/retry story.
...
- **`questionsPerSessionCap`** — stop generating once a session has produced this many real questions, and the sole generation safety rail for live Claude Code sessions (there's no dollar-cost cap — see [What gets sent where, and what it costs](#what-gets-sent-where-and-what-it-costs)). This counts individual displayed questions, not `events` rows: a "both" event (concept + instance) contributes 2 toward the cap, an instance-only (or concept-only) event contributes 1 — so a cap of 8 stops once 8 real questions have been produced, matching what you'll actually see in `grasp review`, not up to double that. Checked before each generation call, not after, so a session can overshoot the cap by at most one extra question (a "both" call made while one question under the cap). When the cap is hit, the `Stop` message says so plainly and points at how to raise it. Dedicated command: `grasp set questions-cap <n> [--global]`. Unrelated to `scanQuestionsCap` below — see [`grasp scan`](#grasp-scan-understanding-existing-code) for why the two must stay independent.
...
- **`grasp scan` re-checks a fully-covered file's content on each run and re-scans it if it's genuinely changed.** Whole-file content hashing, not per-chunk (an edit near the top of a file shifts every later chunk's line numbers, which would make unrelated later chunks falsely look "changed" if compared chunk-by-chunk) — a real edit clears that file's section coverage and it re-enters the walk from the beginning; a trivial edit (reformatting, etc., judged by the same mechanical filter the live diff-capture path uses) just updates the record silently, no re-scan. This only applies to files up to a practical size limit (5,000 lines) — larger files are still scanned/chunked normally, they just don't get this re-check (permanent once fully covered, same as before). Run `grasp reset history` if you want `grasp scan` to look at everything again from scratch regardless.
...
- **A generation call that fails or times out isn't lost — it's retried, not silently dropped.** Generation happens at most once per Claude Code turn (`Stop`), covering everything meaningful captured since the last successful attempt. If that one call errors or times out, Grasp says so directly in its `Stop` message, and the diffs it would have covered simply stay pending — included, combined with whatever else has since accumulated, in the next attempt (this session's next turn), or immediately via `grasp retry` (any session, including ones that have already ended — see [Using it](#using-it)).
