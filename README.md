...
- **`questionsPerSessionCap`** — stop generating once a session has produced this many real question **events**, and the sole generation safety rail (there's no dollar-cost cap — see [What gets sent where, and what it costs](#what-gets-sent-where-and-what-it-costs)). This counts `events` rows, not individual displayed questions: a "both" event (concept + instance) is one toward this cap but shows as two questions in `grasp review`, so a cap of 8 can still leave you with up to 16 questions to actually answer — and since generation is batched once per turn (see [How it works](#how-it-works)), one event can also cover several diffs from the same turn, not just one. When the cap is hit, the `Stop` message says so plainly and points at how to raise it. Dedicated command: `grasp set questions-cap <n> [--global]`. (This is unrelated to any cap a future `grasp scan` command may introduce for scan runs.)
